...
socket.on('message', function(data) {
    console.log("on every message",data);
    getMessage(data);
});
...
function drawNewCard(id, text, x, y, rot, colour, sticker, animationspeed) {
    ...
    card.css('top', startPosition.top - card.height() * 0.25);
    card.css('left', startPosition.left - card.width() * 0.25);
    x=x-40;
    y=y-40;
    card.animate({
        left: x + "px",
...
$(function() {
    ...
    $("#create-card")
        .click(function() {
            console.log("$div",$('div.board-outline').height());
